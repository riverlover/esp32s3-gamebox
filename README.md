...
`.gbc`、`.sfc`、`.smc`、`.md`、`.bin`。约定的目录结构是：

```
/sd/roms/nes/    /sd/roms/gb/    /sd/roms/gbc/    /sd/roms/snes/    /sd/roms/md/
```

但**目录名纯粹是给人看的**，平台按扩展名归类，放错目录照样能玩。想临时下架一批
就挪进 `removed-YYYYMMDD/`——前缀是 `removed` / `_` / `.` 的目录整棵跳过。

⚠️ **压缩包要先在电脑上解开**，设备上不解压（菜单里不会出现，串口会打提示）。
...
ROM 是选中之后才从卡上整个读进 PSRAM 的，所以**卡的速度直接决定进游戏要等多久**。
